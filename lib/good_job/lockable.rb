module Lockable
  ...
  included do
    ...
    def self.advisory_lock_details
      connection.execute("SELECT * FROM pg_locks WHERE locktype = 'advisory' AND objsubid = 1")
    end

    def advisory_lock
      self.class.connection.execute("SELECT 1 as one WHERE pg_try_advisory_lock(('x'||substr(md5('#{id}'), 1, 16))::bit(64)::bigint)").ntuples > 0
    end

    def advisory_lock!
      advisory_lock || raise(RecordAlreadyAdvisoryLockedError)
    end
