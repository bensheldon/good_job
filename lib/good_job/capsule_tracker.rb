...
module GoodJob # :nodoc:
  ...
  class CapsuleTracker
        ...
        if with_advisory_lock
          if @record
            unless advisory_locked?
              ...
              @advisory_locked_connection = advisory_lock_connection ? WeakRef.new(advisory_lock_connection) : nil
            end
          else
    ...
    def matches_advisory_locked_connection?(conn)
      ...
      conn && @advisory_locked_connection.weakref_alive? && @advisory_locked_connection.__getobj__.equal?(conn)
    end
