...
describe GoodJob::CapsuleTracker do
  ...
  describe '#register' do
    context 'when used with an advisory lock' do
      it 'creates a Process and sets the lock_type' do
        ...
        end.to change(GoodJob::CapsuleRecord, :count).by(1)

        process = GoodJob::CapsuleRecord.last
        expect(process.lock_type).to eq('advisory')

        tracker.unregister(with_advisory_lock: true)

        expect(GoodJob::CapsuleRecord.count).to eq 0
      end
      ...
      it 'tracks multiple locks and advisory locks' do
        ...
        expect(GoodJob::CapsuleRecord.count).to eq 1
    ...
    context 'when NOT used with an advisory lock' do
      it 'does not create a Process' do
        expect do
          tracker.register
        end.not_to change(GoodJob::CapsuleRecord, :count)
    ...
    it 'creates a ScheduledTask that refreshes the process in the background' do
      stub_const("GoodJob::CapsuleRecord::STALE_INTERVAL", 0.1.seconds)

      tracker.register do
        tracker.id_for_lock
        updated_at = GoodJob::CapsuleRecord.first.updated_at
        wait_until(max: 1) { expect(GoodJob::CapsuleRecord.first.updated_at).to be > updated_at }
      end
  ...
  describe '.id_for_lock' do
    ...
    it 'changes when the PID changes', :skip_if_java, skip: Gem::Version.new(Rails.version) < Gem::Version.new('6.1.a') && "Fork tracker is broken in tests for Rails v6.0" do
      ...
      # Force the PID check for Active Record to reset
      # and intentionally call `ActiveRecord::ConnectionAdapters::PoolConfig.discard_pools!`
