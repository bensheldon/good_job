...
class CreateGoodJobProcessLockIndexes < ActiveRecord::Migration[7.0]
  ...
  def change
    reversible do |dir|
      dir.up do
        unless connection.index_name_exists?(:good_jobs, :index_good_jobs_dequeue)
          add_index :good_jobs, [:priority, :created_at, :scheduled_at],
                    order: { priority: "ASC NULLS LAST", created_at: :asc, scheduled_at: :asc },
                    where: "finished_at IS NULL AND locked_by_id IS NULL",
                    name: :index_good_jobs_dequeue,
                    algorithm: :concurrently
        end

        unless connection.index_name_exists?(:good_jobs, :index_good_jobs_queue_dequeue)
          add_index :good_jobs, [:queue_name, :priority, :created_at, :scheduled_at],
                    order: { priority: "ASC NULLS LAST", created_at: :asc, scheduled_at: :asc },
                    where: "finished_at IS NULL AND locked_by_id IS NULL",
                    name: :index_good_jobs_queue_dequeue,
                    algorithm: :concurrently
        end
      ...
      dir.down do
        remove_index(:good_jobs, name: :index_good_jobs_dequeue) if connection.index_name_exists?(:good_jobs, :index_good_jobs_dequeue)
        remove_index(:good_jobs, name: :index_good_jobs_queue_dequeue) if connection.index_name_exists?(:good_jobs, :index_good_jobs_queue_dequeue)
