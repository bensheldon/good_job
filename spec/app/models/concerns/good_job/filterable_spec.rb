...
RSpec.describe GoodJob::Filterable do
  ...
  describe '.search_test' do
    ...
    it 'finds results when default_text_search_config is not english' do
      ...
      ActiveRecord::Base.lease_connection.execute("SET default_text_search_config = 'pg_catalog.simple'")
      begin
        expect(model_class.search_text('running')).to include(job_with_stemmed_word)
      ensure
        ActiveRecord::Base.lease_connection.execute("SET default_text_search_config = 'pg_catalog.english'")
      end
    end
  end
end
