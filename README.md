...
- "Optimistic" meaning that the implementation's performance trade-off assumes that collisions are atypical (e.g. two users enqueue the same job at the same time) rather than regular (e.g. the system enqueues thousands of colliding jobs at the same time). You may also want to _manage_ concurrency through the number of GoodJob threads and processes that are performing a given queue.
