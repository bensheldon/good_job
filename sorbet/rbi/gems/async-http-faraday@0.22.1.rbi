# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `async-http-faraday` gem.
# Please instead update this file by running `bin/tapioca gem async-http-faraday`.


# pkg:gem/async-http-faraday#lib/async/http/faraday/version.rb:6
module Async; end

# pkg:gem/async-http-faraday#lib/async/http/faraday/version.rb:7
module Async::HTTP; end

# pkg:gem/async-http-faraday#lib/async/http/faraday/version.rb:8
module Async::HTTP::Faraday; end

# An adapter that allows Faraday to use Async::HTTP as the underlying HTTP client.
#
# pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:98
class Async::HTTP::Faraday::Adapter < ::Faraday::Adapter
  # Create a Faraday compatible adapter.
  #
  # @return [Adapter] a new instance of Adapter
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:124
  def initialize(*_arg0, **_arg1, &_arg2); end

  # Make a request using the adapter.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:155
  def call(env); end

  # Close all clients.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:144
  def close; end

  # Returns the value of attribute read_timeout.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:141
  def read_timeout; end

  # Returns the value of attribute timeout.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:138
  def timeout; end

  private

  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:260
  def encoded_body(response); end

  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:275
  def extract_type_parameters(content_type); end

  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:175
  def perform_request(env); end

  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:232
  def with_client(env); end

  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:250
  def with_timeout(timeout = T.unsafe(nil), task: T.unsafe(nil)); end

  class << self
    # Create a new parallel manager, which is used to handle multiple concurrent requests.
    #
    # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:102
    def setup_parallel_manager(**options); end
  end
end

# The exceptions that are considered connection errors and result in a `Faraday::ConnectionFailed` exception.
#
# pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:107
Async::HTTP::Faraday::Adapter::CONNECTION_EXCEPTIONS = T.let(T.unsafe(nil), Array)

# This is a simple wrapper around Faraday's body that allows it to be read in chunks.
#
# pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:29
class Async::HTTP::Faraday::BodyReadWrapper < ::Protocol::HTTP::Body::Readable
  # Create a new wrapper around the given body.
  #
  # The body must respond to `#read` and `#close` and is often an instance of `IO` or `Faraday::Multipart::CompositeReadIO`.
  #
  # @return [BodyReadWrapper] a new instance of BodyReadWrapper
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:37
  def initialize(body, length = T.unsafe(nil), block_size: T.unsafe(nil)); end

  # Close the body if possible.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:47
  def close(error = T.unsafe(nil)); end

  # Returns the value of attribute length.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:44
  def length; end

  # Read from the body in chunks.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:54
  def read; end
end

# An interface for creating and managing HTTP clients.
#
# pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:17
class Async::HTTP::Faraday::Clients
  # Create a new interface for managing HTTP clients.
  #
  # @return [Clients] a new instance of Clients
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:27
  def initialize(**options, &block); end

  # Close all clients.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:33
  def close; end

  # Make a new client for the given endpoint.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:39
  def make_client(endpoint); end

  # Get a client for the given endpoint.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:49
  def with_client(endpoint); end

  # Get a client for the given proxy endpoint and endpoint.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:62
  def with_proxied_client(proxy_endpoint, endpoint); end

  class << self
    # Create a new instance of the class.
    #
    # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:19
    def call(*_arg0, **_arg1, &_arg2); end
  end
end

# Implement the Faraday parallel manager interface, using Async.
#
# pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:60
class Async::HTTP::Faraday::ParallelManager
  # Create a new parallel manager.
  #
  # @return [ParallelManager] a new instance of ParallelManager
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:62
  def initialize(options = T.unsafe(nil)); end

  # Run the given block asynchronously, using the barrier if available.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:75
  def async(&block); end

  # Execute the given block which can perform multiple concurrent requests, waiting for them all to complete.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:84
  def execute(&block); end

  # @deprecated Please update your Faraday version!
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/adapter.rb:68
  def run; end
end

# An interface for creating and managing per-thread persistent HTTP clients.
#
# pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:144
class Async::HTTP::Faraday::PerThreadPersistentClients
  # Create a new instance of the class.
  #
  # @return [PerThreadPersistentClients] a new instance of PerThreadPersistentClients
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:149
  def initialize(**options, &block); end

  # Close all clients.
  #
  # This will close all clients associated with all threads.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:178
  def close; end

  # Get a client for the given endpoint. If a client already exists for the host, it will be reused.
  #
  # The client instance will be will be cached per-thread.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:161
  def with_client(endpoint, &block); end

  # Get a client for the given proxy endpoint and endpoint. If a client already exists for the host, it will be reused.
  #
  # The client instance will be will be cached per-thread.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:171
  def with_proxied_client(proxy_endpoint, endpoint, &block); end

  private

  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:194
  def clients; end

  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:190
  def make_clients; end
end

# An interface for creating and managing persistent HTTP clients.
#
# pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:74
class Async::HTTP::Faraday::PersistentClients < ::Async::HTTP::Faraday::Clients
  # Create a new instance of the class.
  #
  # @return [PersistentClients] a new instance of PersistentClients
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:76
  def initialize(*_arg0, **_arg1, &_arg2); end

  # Close all clients.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:83
  def close; end

  # Lookup or create a client for the given endpoint.
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:95
  def make_client(endpoint); end

  # Get a client for the given endpoint. If a client already exists for the host, it will be reused.
  #
  # @yield [make_client(endpoint)]
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:106
  def with_client(endpoint); end

  # Get a client for the given proxy endpoint and endpoint. If a client already exists for the host, it will be reused.
  #
  # @yield [proxied_client]
  #
  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:114
  def with_proxied_client(proxy_endpoint, endpoint); end

  private

  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:126
  def fetch(key); end

  # pkg:gem/async-http-faraday#lib/async/http/faraday/clients.rb:132
  def host_key(endpoint); end
end

# pkg:gem/async-http-faraday#lib/async/http/faraday/version.rb:9
Async::HTTP::Faraday::VERSION = T.let(T.unsafe(nil), String)
